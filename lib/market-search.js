...
const PROVIDERS = [
  { id: "daangn", name: "Daangn", accent: "#ff6f0f", description: "Neighborhood marketplace" },
  { id: "joongna", name: "Joongna", accent: "#2f80ed", description: "Nationwide marketplace" },
  { id: "bunjang", name: "Bunjang", accent: "#ef466f", description: "Fast listing discovery" },
];
...
async function scrapeDaangn(query, options = {}) {
  ...
  try {
    ...
    return {
      ...
      items: [],
      rawCount: 0,
      error: error instanceof Error ? error.message : "Daangn search failed.",
      resolvedRegion,
    };
  }
}
...
function parseJoongnaPage(html) {
  ...
  if (!nextData) {
    return { items: [], totalSize: 0, pageCount: 0, error: "__NEXT_DATA__ not found." };
  }
  ...
  if (!searchQuery) {
    return { items: [], totalSize: 0, pageCount: 0, error: "Joongna product data was not found." };
  }
...
async function scrapeJoongna(query) {
  ...
  for (let page = 0; page < MAX_JOONGNA_PAGES; page += 1) {
    try {
      ...
      error = pageError instanceof Error ? pageError.message : "Joongna search failed.";
      break;
    }
  }
...
function mapBunjangItem(rawItem) {
  ...
  return {
    ...
    meta: [cleanText(rawItem.location), rawItem.free_shipping ? "Free shipping" : "", cleanText(rawItem.tag)].filter(Boolean),
  ...
  for (const [variantIndex, variant] of variants.entries()) {
    ...
    for (let page = 1; page <= maxPages; page += 1) {
      try {
        ...
        error = pageError instanceof Error ? pageError.message : "Bunjang search failed.";
        break;
      }
    }
  }
...
export async function searchAllMarkets(options = {}) {
  ...
  const notices = [
    daangn.resolvedRegion
      ? `Daangn was searched with the region "${daangn.resolvedRegion.fullLabel || daangn.resolvedRegion.label}".`
      : "Daangn falls back to its public search surface when no region is selected.",
    "This web version aggregates results on the server before sending them to the page.",
    "Marketplace markup can change, so result counts may vary over time.",
  ];
