...
const MAX_JOONGNA_PAGES = 8;
const MAX_BUNJANG_PAGES = 6;
...
function resolveTargetItems(options = {}, fallback = 180) {
  const explicit = Number(options.targetItems || 0);
  if (explicit > 0) {
    return explicit;
  }
  const requested = Number(options.pageSize || 0);
  if (requested > 0) {
    return Math.min(600, Math.max(120, requested * 3));
  }
  return fallback;
}
...
async function scrapeJoongna(query, options = {}) {
  const provider = PROVIDERS.find((entry) => entry.id === "joongna");
  const items = [];
  let totalSize = 0;
  let pageCount = 0;
  let error = null;
  const maxPages = Math.min(Number(options.maxJoongnaPages || MAX_JOONGNA_PAGES), MAX_JOONGNA_PAGES);
  const targetItems = resolveTargetItems(options, 180);
...
async function scrapeBunjang(query, options = {}) {
  ...
  const variants = buildBunjangQueryVariants(query).slice(0, Math.max(1, Number(options.variantLimit || 2)));
  const maxPages = Math.min(Number(options.maxBunjangPages || MAX_BUNJANG_PAGES), MAX_BUNJANG_PAGES);
  const targetItems = resolveTargetItems(options, 220);

  for (const [variantIndex, variant] of variants.entries()) {
    const variantPageLimit = variantIndex === 0 ? maxPages : Math.min(2, maxPages);
    for (let page = 1; page <= variantPageLimit; page += 1) {
      try {
        const url =
