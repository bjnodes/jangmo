...
const HERO_FEATURES=[
{title:"세 장터 통합 검색",body:"당근마켓, 중고나라, 번개장터 결과를 한 화면에서 비교해요."},
{title:"평균 시세 확인",body:"검색된 가격을 바탕으로 대략적인 시세를 바로 확인해요."},
{title:"지역·가격 필터",body:"당근 지역과 가격 범위를 함께 걸어 원하는 매물만 남겨요."},
];
const HERO_QUICK_SEARCHES=["아이폰 15","브롬톤","닌텐도 스위치","로드 자전거"];
const LABELS={
appSubtitle:"중고거래 통합검색 웹 앱",
search:"검색",
sell:"판매글 작성",
heroEyebrow:"통합 검색",
heroTitle:"모든 중고장터를 한 곳에!",
heroTitleBrand:"장터모아",
...
const pricedItems=items.map((item)=>priceNum(item.price)).filter((value)=>value>0);
const avgPriceLabel=pricedItems.length?`${Math.round(pricedItems.reduce((sum,value)=>sum+value,0)/pricedItems.length).toLocaleString("ko-KR")}\uC6D0`:"-";
